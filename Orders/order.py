...
class order:
    def __init__(self, order_data, is_pending,mt5,local_api,source=None):
        ...
        self.volume = round(order_data.volume_current if is_pending else order_data.volume, 2) if source == "mt5" else order_data.volume
        self.trailing_steps=round(order_data.trailing_steps,2) if source == "db" else 0
        self.Mt5 = mt5
    ...
    def update_from_mt5(self):
        ...
        self.open_time = datetime.datetime.fromtimestamp(order.time_setup if self.is_pending else order.time).strftime('%Y-%m-%d %H:%M:%S')
        ...
        self.volume = round(order.volume_current if self.is_pending else order.volume, 2)
    ...
    def close_order(self):
        ...
        if self.is_pending:
            ...
            self.is_closed = True
            data = self.to_dict()
            self.Mt5.close_order(data,30)
            
        else:
            print(f"Closing order with ticket {self.ticket}")
            self.is_closed = True
            data = self.to_dict()
            self.Mt5.close_position(data,30)
        # update the order 
        self.update_order()
