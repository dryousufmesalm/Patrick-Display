...
class orders_manager:
    ...
    def update_ah_orders_in_db(self):
        for pos in self.all_mt5_orders:
            ...
            if db_order:
                order_obj= order( db_order, db_order.is_pending,self.mt5,self.ah_repo,"db")
    ...
    def get_all_ah_orders_in_db(self):
        self.all_ah_orders= self.ah_repo.get_open_orders_only()
    ...
    def get_suspicious_ah_orders_in_db(self):
        ...
        self.suspious_ah_orders = [order for order in self.all_ah_orders if order not in self.all_mt5_orders]
    ...
    def update_ct_orders_in_db(self):
        for pos in self.all_mt5_orders:
            ...
            # if order exists in database, update it                                
            if db_order:
                order_obj= order( db_order, db_order.is_pending,self.mt5,self.ct_repo,"db")
    ...
    def get_all_ct_orders_in_db(self):
        self.all_ct_orders= self.ct_repo.get_open_orders_only()
        return self.all_ct_orders
    # function that get all suspicious orders in database
    def get_suspicious_ct_orders_in_db(self):
        # get the orders in db orders and not in mt5 orders
        self.suspious_ct_orders = [order for order in self.all_ct_orders if order not in self.all_mt5_orders]
